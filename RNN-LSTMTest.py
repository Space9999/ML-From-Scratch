...
from Layers import RNN, LSTM, Activation, Dense
from Data_Functions import Data_Functions
import numpy as np

def generate_geometric_series(size):
    X = np.zeros([size, 10, 48], dtype = float)
    y = np.zeros([size, 10, 48], dtype = float)
    for i in range(size):
        start = np.random.randint(1, 8)
        geom_series = np.geomspace(start = start, stop = start * 1024, num = 11)
        print(geom_series)
        X[i] = Data_Functions.to_categorical(geom_series, n_col = 48)
        y[i] = np.roll(X[i], -1, axis = 0) # Add some noise by shifting the series around
...
train_error, _ = rnn.fit(X_train, y_train, epochs = 450, batch_size = 500)
train_error_lstm, _ = lstm.fit(X_train, y_train, epochs = 450, batch_size = 500)
...
training = plt.plot(range(450), train_error, label = "RNN Training Error")
training_lstm = plt.plot(range(450), train_error_lstm, label = "LSTM Training Error")
